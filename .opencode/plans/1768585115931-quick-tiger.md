...
Status: Completed in this session. The test was updated to open an in-memory sqlite DB, run migrations, use `sqlstore.New(dbConn)` and construct `entry.NewApp(...)` with the sql-backed store. Tests pass locally (`go test ./...`).
...
- Completed:
  - [x] Ensure logout sets session cookie Secure flag based on TLS
- In progress:
- Completed in this session:
  - [x] Added `/auth/login` templ and generated output
  - [x] Render `/auth/login` using templ in auth handler
  - [x] Canonical logger uses request context with injected user
  - [x] Wire sqlite DB in `cmd/entry-watch/main.go` and run migrations
  - [x] Session cookie Secure flag depends on TLS to allow local HTTP dev
  - [x] Implement sqlite-backed sqlstore and wire into main
  - [x] Migrate `internal/entry/app_test.go` to use in-memory sqlite and `sqlstore`
  - [x] Clear invalid session uid in CanonicalLoggerMiddleware (implemented and committed)
  - [x] Ensure middleware sets session Options Secure flag based on TLS (implemented and committed)
  - [x] Add integration test verifying login sets session cookie
  - [x] Add integration test verifying logout clears session cookie
  - [x] Allow session key to be provided via `ENTRYWATCH_SESSION_KEY` env var (fallback to dev key)
- Blockers / open questions:
  - (fill)

## Plan Status: Finished

This plan has been fully implemented and verified for the current scope described above.

- Finished by: opencode agent
- Finished on: 2026-01-16
- Notes: Created `plan-finished` at the repository root to mark completion and avoid duplicate work in future sessions.
