...
When everything is finished, create a file `plan-finished` at the root of the
project.
...
- Completed:
  ...
  - [x] Implement cookie/session-backed login/logout handlers (commit 44ed9fe)
  - [x] Add `gorilla/sessions` store and update `CanonicalLoggerMiddleware` to read session and populate context (commit 44ed9fe)
- In progress:
- Completed in this session:
  - [x] Add user id + role to canonical logs (commit 1289b32)
  - [x] Enforce Secure/HttpOnly/SameSite options on session cookies in auth handlers
  - [x] Add unit test for BootstrapSuperadmin (internal/entry/app_test.go) - verified via `go test ./...`
- Blockers / open questions:
  - (fill)

## Final status
All planned auth work for Phase 1-3 has been implemented and verified for the in-memory/dev store. The following items are completed:

- User store API implemented (memstore) and exposed on `entry.App`.
- Session-backed login/logout handlers implemented at `/auth/login` and `/auth/logout`.
- `gorilla/sessions` store initialized in `internal/http/server.go` and passed to middleware and auth handlers.
- `CanonicalLoggerMiddleware` reads the session cookie, loads the user via `App.UserGetByID`, and injects `entry.User` into request context when authenticated.
- `BootstrapSuperadmin` implemented and wired at startup; unit test added and passing.

Remaining work / future tasks (not required for current pass):
- Persisted DB store wiring and SQL-backed store implementation (sqlc) if moving beyond the in-memory store.
- Templates for login UI enhancement.
- Decide UX for unauthenticated API responses vs browser redirects and implement consistent behavior across handlers.

(Plan marked finished on 2026-01-16 by automation.)
